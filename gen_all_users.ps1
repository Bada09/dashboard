...
$outputPath = "c:\Users\badad\Downloads\metlife proposta\full_users_list.js"
...
if (Test-Path $filePath) {
    ...
    foreach ($member in $raw.members) {
        ...
        if ($fLocales.Count -eq 0) { $fLocales = @("N/A") }
        ...
        $userList += @{
            ...
            insights = @{
                pt = if ($ptFeedback.Length -gt 10) { ($ptFeedback -replace '[\r\n\t]', ' ' -replace '"', "'").Substring(0, [Math]::Min(300, $ptFeedback.Length)) + "..." } else { "Aguardando treinamento." }
                fr = if ($ptFeedback.Length -gt 10) { ($ptFeedback -replace '[\r\n\t]', ' ' -replace '"', "'").Substring(0, [Math]::Min(300, $ptFeedback.Length)) + "..." } else { "En attente de formation." }
            }
            improvement = @{
                pt = if ($improvements_pt.Count -gt 0) { ($improvements_pt | Select-Object -Unique) -join ", " } else { "Iniciar treinamento LQA" }
                fr = if ($improvements_fr.Count -gt 0) { ($improvements_fr | Select-Object -Unique) -join ", " } else { "Commencer la formation LQA" }
            }
